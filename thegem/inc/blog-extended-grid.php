...
if (!function_exists('thegem_extended_blog_render_item')) {
	function thegem_extended_blog_render_item($params, $item_classes, $thegem_sizes = null, $post_id = false) {
		...
		if ($post_id) {
			...
			if (($params['ignore_highlights'] != '1') && !empty($post_item_data['highlight'])) {
...
if (!function_exists('blog_grid_extended_more_callback')) {
	function blog_grid_extended_more_callback() {
		...
		if ($page == 0)
			$page = 1;
		$news_grid_loop = get_thegem_extended_blog_posts($settings['categories'], $page, $settings['items_per_page'], $settings['orderby'], $settings['order']);
		...
		<div data-page="<?php echo $page; ?>" data-next-page="<?php echo $next_page; ?>"
			 data-pages-count="<?php echo esc_attr($news_grid_loop->max_num_pages); ?>">
			<?php
...
if (!function_exists('get_thegem_extended_blog_posts')) {
	function get_thegem_extended_blog_posts($news_cat, $page = 1, $ppp = -1, $orderby = 'menu_order date', $order = 'DESC') {
		if (empty($news_cat)) {
			return null;
		}
...
if (!function_exists('thegem_search_grid_content')) {
	function thegem_search_grid_content($mixed_grid = false) {
		...
		$params = array(
			...
			'blog_title_preset' => thegem_get_option('search_layout_caption_title_preset'),
