...
function thegem_blog($params = array()) {
	$params = array_merge(array(
		...
		'image_size' => '',
		'image_border_radius' => '',
		'title_preset1' => 'main-menu-item',
		'title_preset2' => 'text-body-tiny',
		'title_transform' => '',
	...
	if (isset($params['source']) && $params['source'] == 'tags') {
	...
	} else if (isset($params['source']) && $params['source'] == 'posts') {
		$args['post__in'] = explode(',', $params['blog_posts']);
		$args['ignore_sticky_posts'] = 1;
	} else if (!empty($params['blog_categories']) && !in_array('--all--', $params['blog_categories'])) {
		$args['tax_query'] = array(
			'relation' => 'OR',
			array(
				'taxonomy' => 'category',
	...
	if (!empty($params['blog_offset'])) {
		$args['offset'] = $params['blog_offset'];
	}
	...
	if (!wp_style_is('thegem-blog', 'done')) {
		wp_register_style('thegem-blog', get_template_directory_uri() . '/css/thegem-blog.css');
		wp_print_styles(array('thegem-blog'));
		wp_deregister_style('thegem-blog');
		wp_dequeue_style('thegem-blog');
	}
