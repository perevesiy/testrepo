...
<?php if (!isset($portfolio_item_size)):
	...
	<div <?php post_class($thegem_classes); ?> <?php if (isset($gap_size)) { ?>style="padding: <?php echo intval($gap_size); ?>px;" <?php } ?> data-default-sort="<?php echo intval(get_post()->menu_order); ?>" data-sort-date="<?php echo get_the_date('U'); ?>">
				...
				<div class="highlight-item-alternate-box-content caption">
					<div class="highlight-item-alternate-box-content-inline">
						...
						<?php if (!isset($params['blog_show_title']) || $params['blog_show_title']) {
							if (isset($params['blog_title_preset']) && $params['blog_title_preset'] != 'default') {
								$title = $params['blog_title_preset'];
							} else if ($params['version'] == 'new' ) {
								$title = 'h4';
							} else {
								$title = 'h6';
							} ?>
							<div class="title">
								<?php the_title('<div class="title-' . $title . '">', '</div>'); ?>
							</div>
						<?php } ?>
		...
		<div class="wrap clearfix">
			<?php if (isset($params['post_type_indication']) && $params['post_type_indication'] == '1') {
			...
			<?php if ($params['display_titles'] == 'hover' || !$params['hide_featured_image']) { ?>
				<div <?php post_class($thegem_image_classes); ?>>
					<div class="image-inner <?php echo $thegem_has_post_thumbnail ? '' : 'without-image'; ?>">
						<?php if ($params['layout'] == 'justified' || (!$thegem_has_post_thumbnail && isset($params['search']) && !in_array($post_format, array('quote', 'audio', 'video'))) ): ?>
												...
												<div class="slide-content-visible">
													...
													<?php if (!isset($params['blog_show_title']) || $params['blog_show_title']) {
														if (isset($params['blog_title_preset']) && $params['blog_title_preset'] != 'default') {
															...
															$title = isset($post_item_data['highlight']) && $post_item_data['highlight'] && $params['layout'] != 'metro' && empty($params['is_slider']) && $thegem_highlight_type == 'squared' ? 'h4' : 'h5';
														} ?>
														<div class="title">
															<?php the_title('<div class="title-' . $title .'">', '</div>'); ?>
														</div>
			...
			<?php if ( $params['display_titles'] == 'page' && $post_format != 'quote'): ?>
				<div <?php post_class($thegem_caption_classes); ?>>
					...
					<?php if (!isset($params['blog_show_title']) || $params['blog_show_title']) { ?>
						<div class="title">
							<?php if (isset($params['blog_title_preset']) && $params['blog_title_preset'] != 'default') {
								$title = $params['blog_title_preset'];
							...
							<?php the_title('<div class="title-' . $title . '"><a href="' . esc_url(get_permalink()) . '" rel="bookmark">', '</a></div>'); ?>
						</div>
					<?php } ?>
