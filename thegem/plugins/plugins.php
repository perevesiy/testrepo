...
function thegem_pre_set_site_transient_update_plugins( $transient ) {
	...
	$response = wp_remote_get('http://democontent.codex-themes.com/plugins/thegem/recommended/wp-rocket.json', array('timeout' => 5));
	...
	$wp_rocket_data = new stdClass;

	if(isset($transient->response['wp-rocket/wp-rocket.php'])) {
		$wp_rocket_data = $transient->response['wp-rocket/wp-rocket.php'];
		unset($transient->response['wp-rocket/wp-rocket.php']);
	} elseif(isset($transient->no_update['wp-rocket/wp-rocket.php'])) {
		$wp_rocket_data = $transient->no_update['wp-rocket/wp-rocket.php'];
		unset($transient->no_update['wp-rocket/wp-rocket.php']);
	}

	if(defined( 'WP_ROCKET_VERSION' ) && isset($wp_rocket_data->new_version)) {
		if ( version_compare( WP_ROCKET_VERSION, $new_version, '<' ) ) {
			$wp_rocket_data->new_version = $new_version;
			$wp_rocket_data->package = $data['package'];
			$transient->response['wp-rocket/wp-rocket.php'] = $wp_rocket_data;
		} else {
			$wp_rocket_data->new_version = $new_version;
			$wp_rocket_data->package = $data['package'];
			$transient->no_update['wp-rocket/wp-rocket.php'] = $wp_rocket_data;
		}
	}
...
function thegem_plugins_update_latest_version_notice() {
	...
	if(thegem_is_plugin_active('thegem-elements/thegem-elements.php')) {
		$plugin_data = get_plugin_data(trailingslashit(WP_PLUGIN_DIR).'thegem-elements/thegem-elements.php');
		if(version_compare($plugin_data['Version'], '5.5.0', '<')) {
	...
	if(thegem_is_plugin_active('thegem-importer/thegem-importer.php')) {
		$plugin_data = get_plugin_data(trailingslashit(WP_PLUGIN_DIR).'thegem-importer/thegem-importer.php');
		if(version_compare($plugin_data['Version'], '5.3.0', '<')) {
			echo '<div class="thegem-update-notice-new notice notice-error" style="display: flex; align-items: center;">';
			echo '<p style="margin: 5px 15px 0 10px;"><img src=" '.THEGEM_THEME_URI . '/images/alert-icon.svg'.' " width="40px" alt="thegem-blocks-logo"></p>';
...
function thegem_install_optimizers() {
	...
	$plugin_info->package = 'http://democontent.codex-themes.com/plugins/thegem/recommended/wp-rocket.zip';
	$plugin_info->url = '';
	$plugins_updates->response['wp-rocket/wp-rocket.php'] = $plugin_info;
	...
	$plugin_info->package = 'https://downloads.wordpress.org/plugin/autoptimize.zip';
	$plugin_info->url = '';
	...
	$upgrader = new Plugin_Upgrader( new Plugin_Upgrader_Skin( $upgrader_args ) );
	$installed = $upgrader->bulk_upgrade( ['wp-rocket/wp-rocket.php', 'wp-rocket-disable-google-font-optimization/wp-rocket-disable-google-font-optimization.php', 'autoptimize/autoptimize.php'] );
	if ($installed) {
		unset($plugins_updates->response['wp-rocket/wp-rocket.php']);
		...
		if(!is_wp_error($response_p)) {
			...
			if(is_array($rp_data) && isset($rp_data['wp_rocket_settings'])) {
				$wpRocketSettings = array_merge($wpRocketSettings, $rp_data['wp_rocket_settings']);
				delete_option('thegem_optimizer_error');

				for($try=0; $try<4; $try++) {
					$wp_rocket_response = wp_remote_get('https://wp-rocket.me/valid_key.php', array(
						'timeout' => 30,
						'user-agent' => 'WordPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ). sprintf( ';WP-Rocket|%s%s|%s|%s|%s|;', $wpRocketSettings['version'], '', 
							$wpRocketSettings['consumer_key'], $wpRocketSettings['consumer_email'], esc_url( home_url() ) )
					));

					if ( !is_wp_error( $wp_rocket_response ) ) {
						$body = wp_remote_retrieve_body( $wp_rocket_response );
